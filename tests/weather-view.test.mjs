...
test('current weather is labelled as current, with city-local model time and retrieval time',()=>{
  const html=feature('currentMarkup')('paris',state,now);
  for(const text of ['巴黎','20','体感','12:00','获取','当前'])assert.ok(html.includes(text),text);
});
test('null rain probability is rendered as missing, not as zero',()=>{
  ...
  assert.ok(html.includes('降雨概率：—')); assert.ok(!html.includes('0%'));
  const zero=structuredClone(state);zero.data.days[0].rainProbability=0;
  assert.ok(render('2026-09-27','paris',zero,now).includes('降雨概率：0%'));
});
...
test('stale cache is visibly identified when refresh fails',()=>{
  ...
  assert.ok(feature('currentMarkup')('paris',stale,now).includes('缓存'));
...
test('weather failure produces a usable fallback, not a loading state forever',()=>{
  const html=feature('currentMarkup')('paris',{data:null,stale:false,fetchedAt:null,error:'offline'},now);
  assert.ok(html.includes('暂时无法获取'));assert.ok(html.includes('行程'));
});
